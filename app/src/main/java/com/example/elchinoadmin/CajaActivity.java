...
public class CajaActivity extends AppCompatActivity {
    ...
    private String sheet_caja = "caja";
    ...
    private Map<String, String> cajas = new HashMap<String, String>();
    ...
    private void mostrar_todo (String cobrador) {
        /*
        ...
        et_monto_recuperado.setText(String.valueOf(cajas.get(cobrador)) + " colones");
        tv_monto_recuperado.setText("Caja " + cobrador + ": ");
    ...
    private void revisar_cajas_de_hoy () {
        ocultar_todo("Obteniendo ventas de hoy...");

        if (cobradores.isEmpty()) {
            //Do nothing.
            mostrar_todo("sin cobrador");
            llenar_spinner();
            //leer_cobros_de_hoy();
        } else {
    ...
    private void llenar_spinner () {
        ...
        String cobradores = "Escoja el cobrador...___";

        for (String key : cajas.keySet()) {
            cobradores = cobradores + key + "___";

        }
        Log.v("llenando_spinner-1", "Caja.\n\nCcobradores:\n\n" + cobradores + "\n\n.");
        String[] split_spinner = cobradores.split("___");
    ...
    private void leer_caja_nube (String key, String value) {
...
split2[47]: },{
        ...
        //sheet_abonos = "abonos";
        ocultar_todo("Leyendo caja de\n" + apodo_cobrador + ", por favor espere...");
        ...
        if (verificar_internet()) {
            ...
            String url = readRowURL + spreadsheet_creditos + "&sheet=" + sheet_caja;

            Log.v("verificar_caja()", ".\nurl: " + url + "\n.");
                    ...
                    new Response.Listener<String>() {
                        ...
                        public void onResponse(String response) {
                            ...
                            if (response != null) {
                                ...
                                String[] split = response.split("caja");
                                if (split.length > 0) {
                                    for (int i = 0; i < split.length; i++) {
                                        String[] split2 = split[i].split("\"");
                                        Log.v("leer_caja_nube0", "CajaActivity.\n\nSplit.length: " + split.length + "\n\nsplit[" + i + "]: " + split[i] + "\n\n.");
                                        if (split2.length > 1) {
                                            String linea_hash = split2[0] + "_sepa_linea_" + split2[2] + "_sepa_linea_" + apodo_cobrador;
                                            Log.v("leer_caja_nube1", "Caja.\n\nlinea_hash: " + linea_hash + "\n\n.");
                                            if (lineas.containsValue(linea_hash)) {
                                                //Do nothing. Aqui se eliminan los archivos repetidos.
                                            } else {
                                                Log.v("leer_caja_nube2", "Caja.\n\nLinea hash: " + "\n\n" + linea_hash + "\n\n.");
                                                lineas.put(i, linea_hash);
                                                if (cajas.containsKey(apodo_cobrador)) {
                                                    cajas.replace(apodo_cobrador, cajas.get(apodo_cobrador), split2[2]);
                                                } else {
                                                    cajas.put(apodo_cobrador, split2[2]);
                                                    //cajas_total = Integer.parseInt(split2[2]);
